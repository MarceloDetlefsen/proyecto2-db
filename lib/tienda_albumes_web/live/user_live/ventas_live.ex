defmodule TiendaAlbumesWeb.VentasLive do
  ...
  def handle_event("guardar_venta", params, socket) do
    ...
    result =
      Repo.transaction(fn ->
        ...
        case Repo.query("""
          ...
          RETURNING id_compra
        """, [fecha, id_cliente, id_empleado]) do
          {:ok, %{rows: [[id_compra]]}} ->
    ...
    case result do
      ...
      {:error, {:stock_insuficiente, id}} ->
        {:noreply, put_flash(socket, :error, "Stock insuficiente para el producto ##{id}. Operación revertida (ROLLBACK).")}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Error al registrar la venta. Operación revertida (ROLLBACK).")}
    end
  end
  ...
  def handle_event("eliminar_venta", %{"id" => id}, socket) do
    ...
    Repo.transaction(fn ->
      {:ok, %{rows: items}} =
        Repo.query("SELECT id_producto, cantidad FROM detalle_compra WHERE id_compra = $1", [id_int])
      Enum.each(items, fn [id_prod, cant] ->
        Repo.query("UPDATE producto SET stock = stock + $1 WHERE id_producto = $2", [cant, id_prod])
      end)
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 style="font-family: Georgia, serif; font-size: 1.8rem; font-weight: 700; color: #385404;">Ventas</h1>
        </div>
        <button phx-click="nueva_venta" class="btn btn-sm"
      ...
      <form phx-change="filtrar" phx-submit="filtrar"
        ...
        <div>
          <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Cliente</label>
        ...
        <div>
          <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Empleado</label>
        ...
        <div>
          <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Desde</label>
          <input type="date" name="fecha_desde" value={@filtros["fecha_desde"]}
            class="input input-sm w-full"
            style="background-color: #f7fbf6; border-color: #c8d4a0; color: #385404;" />
        </div>
        <div>
          <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Hasta</label>
          <input type="date" name="fecha_hasta" value={@filtros["fecha_hasta"]}
            class="input input-sm w-full"
            style="background-color: #f7fbf6; border-color: #c8d4a0; color: #385404;" />
        </div>
        <div class="flex items-end">
          <button type="button" phx-click="limpiar_filtros" class="btn btn-sm w-full"
          ...
          <tbody>
            <%= for v <- @ventas do %>
              <tr style="border-bottom: 1px solid #e2e8d5;">
                ...
                <td style="font-family: Georgia, serif; color: #385404; font-size: 13px;">{v.cliente}</td>
      ...
      <%= if @modal == :detalle && @venta_detalle do %>
        <div class="fixed inset-0 flex items-center justify-center z-50"
          ...
          <div class="rounded-box border p-6 w-full max-w-2xl shadow-xl"
            ...
            <div class="flex items-start justify-between mb-4">
              <div>
                ...
                <p style="font-size: 11px; color: #97a77d; margin-top: 2px;">
                  {(@venta_detalle.venta && to_string(@venta_detalle.venta.fecha))} ·
            ...
            <table class="table table-sm w-full mb-4" style="background-color: #f7fbf6;">
              ...
              <tbody>
                <%= for item <- @venta_detalle.items do %>
                  <tr style="border-bottom: 1px solid #e2e8d5;">
                    <td style="font-family: Georgia, serif; font-weight: 600; color: #385404; font-size: 13px;">{item.titulo}</td>
                    <td style="color: #6a7a54; font-size: 12px;">{item.artista}</td>
                    <td>
                      <span class="badge badge-sm"
                        style={if item.formato == "Vinilo",
                    ...
                    <td style="font-weight: 700; color: #385404; font-size: 13px;">${item.subtotal}</td>
                  </tr>
                <% end %>
              </tbody>
            </table>
          ...
          <div class="rounded-box border p-6 w-full max-w-2xl shadow-xl overflow-y-auto"
            ...
            <form phx-submit="guardar_venta">
              ...
              <div class="grid grid-cols-3 gap-3 mb-5">
                <div>
                  <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Fecha</label>
                ...
                <div>
                  <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Cliente</label>
                  <select name="id_cliente" required class="select select-sm w-full"
                    style="background-color: #f1f5eb; border-color: #c8d4a0; color: #385404;">
                ...
                <div>
                  <label style="font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #97a77d; display: block; margin-bottom: 4px;">Empleado</label>
                  <select name="id_empleado" required class="select select-sm w-full"
                    style="background-color: #f1f5eb; border-color: #c8d4a0; color: #385404;">
                    <option value="">Seleccionar...</option>
                    <%= for {nombre, id} <- @empleados do %>
                      <option value={id}>{nombre}</option>
              ...
              <%= for {_item, idx} <- Enum.with_index(@items_nueva_venta) do %>
                <div class="flex gap-2 mb-2 items-end">
                  <div class="flex-1">
                    <label style="font-size: 9px; color: #97a77d; display: block; margin-bottom: 3px;">Producto</label>
                  ...
                  <div style="width: 80px;">
                    <label style="font-size: 9px; color: #97a77d; display: block; margin-bottom: 3px;">Cant.</label>
                    <input type="number" name={"cantidad_#{idx}"} min="1" value="1"
                      class="input input-sm w-full"
                      style="background-color: #f1f5eb; border-color: #c8d4a0; color: #385404;" />
                  </div>
                  <button type="button" phx-click="quitar_item" phx-value-idx={idx}
                    class="btn btn-sm btn-ghost mb-0"
                    style="color: #a33a2a;">✕</button>
                </div>
              <% end %>

              <button type="button" phx-click="agregar_item" class="btn btn-sm w-full mb-5"
                style="background-color: #e2e8d5; border-color: #c8d4a0; color: #385404;">
                + Agregar producto
              </button>
